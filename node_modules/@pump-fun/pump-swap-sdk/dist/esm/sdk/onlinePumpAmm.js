...
function depositToken0(token0, slippage, token0Reserve, token1Reserve, totalLpTokens) {
  ...
  const slippageFactor = new BN4((1 + slippage / 100) * 1e9);
  const maxToken0 = token0.mul(slippageFactor).div(new BN4(1e9));
  const maxToken1 = token1.mul(slippageFactor).div(new BN4(1e9));
...
function ceilDiv2(numerator, denominator) {
  return numerator.add(denominator).sub(new BN4(1)).div(denominator);
}
function depositLpToken(lpToken, slippage, baseReserve, quoteReserve, totalLpTokens) {
  ...
  const slippageFactor = new BN4((1 + slippage / 100) * 1e9);
  const slippageDenominator = new BN4(1e9);
...
function buyBaseInput({
  ...
  const {
    ...
    feeConfig,
    creator,
    baseMintSupply: new BN6(baseMintAccount.supply.toString()),
    baseMint,
    baseReserve,
    quoteReserve,
  ...
  const coinCreatorFee = PublicKey2.default.equals(coinCreator) ? new BN6(0) : fee(quoteAmountIn, coinCreatorFeeBps);
...
function buyQuoteInput({
  ...
  const totalFeeBps = lpFeeBps.add(protocolFeeBps).add(PublicKey2.default.equals(coinCreator) ? new BN6(0) : coinCreatorFeeBps);
  const denominator = new BN6(1e4).add(totalFeeBps);
  const effectiveQuote = quote.mul(new BN6(1e4)).div(denominator);
...
function sellBaseInput({
  ...
  const coinCreatorFee = PublicKey3.default.equals(coinCreator) ? new BN7(0) : fee(quoteAmountOut, coinCreatorFeeBps);
...
var MAX_FEE_BASIS_POINTS = new BN7(1e4);
...
function sellQuoteInput({
  ...
  const rawQuote = calculateQuoteAmountOut(
    ...
    lpFeeBps,
    protocolFeeBps,
    PublicKey3.default.equals(coinCreator) ? new BN7(0) : coinCreatorFeeBps
  );
...
var OnlinePumpAmmSdk = class {
  ...
  async swapSolanaState(poolKey, user, userBaseTokenAccount = void 0, userQuoteTokenAccount = void 0) {
    ...
    return {
      ...
      poolAccountInfo,
      pool,
      poolBaseAmount: new BN9(decodedPoolBaseTokenAccount.amount.toString()),
      poolQuoteAmount: new BN9(decodedPoolQuoteTokenAccount.amount.toString()),
      baseTokenProgram,
      quoteTokenProgram,
      baseMint,
  ...
  async getCoinCreatorVaultBalance(coinCreator) {
    ...
    try {
      ...
      return new BN9(tokenAccount.amount.toString());
    } catch (e) {

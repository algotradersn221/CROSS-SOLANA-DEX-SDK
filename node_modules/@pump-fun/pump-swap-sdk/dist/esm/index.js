...
var PUMP_AMM_PROGRAM_ID = new PublicKey(
  "pAMMBay6oceH9fJKBRHGP5D4bD4sWpmSwMn52FMfXEA"
);
var PUMP_FEE_PROGRAM_ID = new PublicKey(
...
function poolPda(index, owner, baseMint, quoteMint) {
  return pumpAmmPda([
    ...
    new BN(index).toArrayLike(import_buffer.Buffer, "le", 2),
    owner.toBuffer(),
    baseMint.toBuffer(),
    quoteMint.toBuffer()
  ]);
...
var PumpAmmAdminSdk = class {
  ...
  async adminUpdateTokenIncentives(startTime, endTime, dayNumber, tokenSupplyPerDay, secondsInADay = new BN3(86400), mint = PUMP_MINT, tokenProgram = TOKEN_2022_PROGRAM_ID2) {
...
function totalUnclaimedTokens(globalVolumeAccumulator, userVolumeAccumulator, currentTimestamp = Date.now() / 1e3) {
  ...
  let currentTimestampBn = new BN4(currentTimestamp);
...
function depositToken0(token0, slippage, token0Reserve, token1Reserve, totalLpTokens) {
  ...
  const slippageFactor = new BN5((1 + slippage / 100) * 1e9);
  const maxToken0 = token0.mul(slippageFactor).div(new BN5(1e9));
  const maxToken1 = token1.mul(slippageFactor).div(new BN5(1e9));
...
function ceilDiv2(numerator, denominator) {
  return numerator.add(denominator).sub(new BN5(1)).div(denominator);
}
function depositLpToken(lpToken, slippage, baseReserve, quoteReserve, totalLpTokens) {
  ...
  const slippageFactor = new BN5((1 + slippage / 100) * 1e9);
  const slippageDenominator = new BN5(1e9);
...
function withdraw(lpAmount, slippage, baseReserve, quoteReserve, totalLpTokens) {
  ...
  const scaleFactor = new BN6(1e9);
  const slippageFactor = new BN6((1 - slippage / 100) * 1e9);
...
function buyBaseInput({
  ...
  const {
    ...
    feeConfig,
    creator,
    baseMintSupply: new BN7(baseMintAccount.supply.toString()),
    baseMint,
    baseReserve,
    quoteReserve,
  ...
  const coinCreatorFee = PublicKey2.default.equals(coinCreator) ? new BN7(0) : fee(quoteAmountIn, coinCreatorFeeBps);
  const totalQuote = quoteAmountIn.add(lpFee).add(protocolFee).add(coinCreatorFee);
  const precision = new BN7(1e9);
  const slippageFactorFloat = (1 + slippage / 100) * 1e9;
  const slippageFactor = new BN7(Math.floor(slippageFactorFloat));
...
function buyQuoteInput({
  ...
  const totalFeeBps = lpFeeBps.add(protocolFeeBps).add(PublicKey2.default.equals(coinCreator) ? new BN7(0) : coinCreatorFeeBps);
  const denominator = new BN7(1e4).add(totalFeeBps);
  const effectiveQuote = quote.mul(new BN7(1e4)).div(denominator);
...
function sellBaseInput({
  ...
  const {
    ...
    feeConfig,
    creator,
    baseMintSupply: new BN8(baseMintAccount.supply.toString()),
    baseMint,
    baseReserve,
    quoteReserve,
  ...
  const coinCreatorFee = PublicKey3.default.equals(coinCreator) ? new BN8(0) : fee(quoteAmountOut, coinCreatorFeeBps);
  ...
  const precision = new BN8(1e9);
  const slippageFactorFloat = (1 - slippage / 100) * 1e9;
  const slippageFactor = new BN8(Math.floor(slippageFactorFloat));
...
var MAX_FEE_BASIS_POINTS = new BN8(1e4);
...
function sellQuoteInput({
  ...
  const rawQuote = calculateQuoteAmountOut(
    ...
    lpFeeBps,
    protocolFeeBps,
    PublicKey3.default.equals(coinCreator) ? new BN8(0) : coinCreatorFeeBps
  );
...
var PumpAmmSdk = class {
    ...
    return await this.withFixPoolInstructions(
      ...
      async () => {
        return await this.withWsolAccounts(
          ...
          async () => {
            const instructions = [];
            if (!this.accountExists(userPoolAccountInfo, TOKEN_2022_PROGRAM_ID3)) {
              instructions.push(
                createAssociatedTokenAccountIdempotentInstruction(
                  user,
                  userPoolTokenAccount,
  ...
  depositBaseInput(liquiditySolanaState, base, slippage) {
    ...
    const { token1, lpToken, maxToken0, maxToken1 } = depositToken0(
      base,
      slippage,
      new BN9(poolBaseTokenAccount.amount.toString()),
      new BN9(poolQuoteTokenAccount.amount.toString()),
      pool.lpSupply
    );
    return {
      quote: token1,
...
var OnlinePumpAmmSdk = class {
  ...
  async createPoolSolanaState(index, creator, baseMint, quoteMint, userBaseTokenAccount = void 0, userQuoteTokenAccount = void 0) {
    ...
    const poolBaseTokenAccount = getAssociatedTokenAddressSync3(
      baseMint,
      poolKey,
      true,
      baseTokenProgram
    );
    const poolQuoteTokenAccount = getAssociatedTokenAddressSync3(
      quoteMint,
      poolKey,
      true,
      quoteTokenProgram
    ...
    if (userBaseTokenAccount === void 0) {
      userBaseTokenAccount = getAssociatedTokenAddressSync3(
        baseMint,
        creator,
        true,
        baseTokenProgram
    ...
    if (userQuoteTokenAccount === void 0) {
      userQuoteTokenAccount = getAssociatedTokenAddressSync3(
        quoteMint,
        creator,
        true,
        quoteTokenProgram
  ...
  async swapSolanaState(poolKey, user, userBaseTokenAccount = void 0, userQuoteTokenAccount = void 0) {
    ...
    return {
      ...
      poolAccountInfo,
      pool,
      poolBaseAmount: new BN10(decodedPoolBaseTokenAccount.amount.toString()),
      poolQuoteAmount: new BN10(decodedPoolQuoteTokenAccount.amount.toString()),
      baseTokenProgram,
      quoteTokenProgram,
      baseMint,
  ...
  async liquiditySolanaState(poolKey, user, userBaseTokenAccount = void 0, userQuoteTokenAccount = void 0, userPoolTokenAccount = void 0) {
    ...
    if (userPoolTokenAccount === void 0) {
      userPoolTokenAccount = getAssociatedTokenAddressSync3(
        lpMint,
        user,
        true,
        TOKEN_2022_PROGRAM_ID4
      );
    }
  ...
  async collectCoinCreatorFeeSolanaState(coinCreator, coinCreatorTokenAccount = void 0) {
    const quoteMint = NATIVE_MINT3;
    const quoteTokenProgram = TOKEN_PROGRAM_ID2;
    ...
    if (coinCreatorTokenAccount === void 0) {
      coinCreatorTokenAccount = getAssociatedTokenAddressSync3(
        quoteMint,
        coinCreator,
        true,
        quoteTokenProgram
  ...
  async getCoinCreatorVaultBalance(coinCreator) {
    ...
    try {
      ...
      return new BN10(tokenAccount.amount.toString());
    } catch (e) {
  ...
  async claimTokenIncentives(user, payer) {
    ...
    if (mint.equals(PublicKey5.default)) {
      return [];
    }

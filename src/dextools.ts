...
export async function AllPools(
    ...
    try {
        const res = await fetch(url);
        if (!res.ok) return [tokenAddress, []];

        const data = (await res.json()) as PoolDatas[];

        const filtered: PoolDatas[] = data.filter(
            (pool) =>
                (dexIds.includes(pool.dexId) || dexIds.length === 0) &&
                (pool.liquidity?.usd ?? 0) >= minUsdLiquidity &&
                (pool.priceUsd ?? 0) > 0
        );

        return [tokenAddress, filtered];
    } catch (err) {
        console.error(`Erreur fetchPools pour ${tokenAddress}:`, err);
        return [tokenAddress, []];
    }
}
